			...
			for (int St = 0; St < GlobalConstants.total_Students; St++) {
				...
				for (int A = 1; A < Utils.getLast(S); A++) {
					...
					for (int list_K = 0; list_K < KCs.size(); list_K++) {
						...
						// System.out.println((A+1)+" "+Utils.getBest(S, Kc, A +
						// 1)+" , "+A+" "+Utils.getBest(S, Kc, A));
						LearnNumerator = Operations.addDouble(LearnNumerator,
								Operations.substractDouble(Utils.getBest(S, Kc, A + 1), Utils.getBest(S, Kc, A)));
						LearnDenominator = Operations.addDouble(LearnDenominator, Operations
								.multiplyDouble(Operations.substractDouble((double) 1, Utils.getBest(S, Kc, A)), SE));
						// System.out.println(St+" "+A+" "+list_K);
						// System.out.println("LearnNumerator
						// :"+LearnNumerator+" =
						// "+LearnNumerator+"+"+Operations.substractDouble(Utils.getBest(S,
						// Kc, A + 1),Utils.getBest(S, Kc, A)));
						// System.out.println("LearnDenominator
						// :"+LearnDenominator+" =
						// "+LearnDenominator+"+"+Operations.multiplyDouble(Operations.substractDouble((double)1,Utils.getBest(S,
						// Kc, A)),SE)); }
					}
				}
				...
				//System.out.println(
				//		"setLearnMap :" + Math.min(Double.valueOf(0.5), max) + "  " + Double.valueOf(0.5) + " " + max);
