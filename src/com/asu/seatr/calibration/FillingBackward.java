...
public class FillingBackward {
	...
	private static ArrayList<Integer> KCs = new ArrayList<Integer>();
		...
		for (int St = 0; St < Ns; St++) {
		 ...
		 System.out.println("Student ------------"+St);
			...
			for (int A = Utils.getLast(S) - 1; A >= 1; A--) {
				...
				KCs.clear();
				KCs = Utils.getQuestionMatrix(question);
				Double SE = (double) 1;
				for (int list_K = 0; list_K < KCs.size(); list_K++) {
					SE = Operations.multiplyDouble(SE, Utils.getBackward(S, KCs.get(list_K), A + 1));
					// System.out.println("SE :"+SE+" * Utils.getBackward("+S+",
					// "+KCs.get(list_K)+","+( A + 1)+") :"+Utils.getBackward(S,
					// KCs.get(list_K), A + 1));
				}
				Double x;
				for (int list_K = 0; list_K < KCs.size(); list_K++) {
