			...
			for (int S = 0; S < GlobalConstants.total_Students ; S++) {
				...
				for (int A = 0; A < Utils.getLast(S); A++) {
					...
					if (Q == Utils.getQuestion(S, A)) {
						...
						Utils.mSlip[Q] = SlipNumerator.divide(SlipDenominator,20,RoundingMode.HALF_UP);
						System.out.println("Utils.mSlip[Q]"+Utils.mSlip[Q]);
						Utils.mGuess[Q] = GuessNumerator.divide(GuessDenominator,20,RoundingMode.HALF_UP);
						System.out.println("Utils.mGuess[Q]"+Utils.mGuess[Q]);
					}
				}
			}
